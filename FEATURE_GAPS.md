...
15. Machine pixel streams are split between the legacy helper path and an early Moonlight gateway path.

   homelab now runs a local MediaMTX WebRTC/RTSP router, and wmux can show per-machine stream paths from the Stream button. `wmux-stream-agent` can run as a long-lived helper and only starts capture while wmux has an active stream-viewer lease, but this is still a helper process that must run in the graphical login session of each participating machine. Windows FFmpeg/gdigrab publishing through the per-user Scheduled Task is validated on 9800x3d, including on-demand start/idle behavior. macOS still requires Screen Recording permission for the app/process that owns capture, Wayland capture is not implemented, and locked/logged-out Windows behavior still needs explicit validation.

   `wmux-moonlight-gateway` is now available as an alternate stream provider that proxies a browser-native Moonlight bridge and reports gateway health to wmux. It can automate the supported pairing path by generating the Moonlight Web PIN and submitting it to Sunshine's `/api/pin`, and the Windows setup helper can install/configure/start Sunshine. It still does not implement the Moonlight/GameStream protocol, app launch, codec negotiation, stream settings, or input transport itself. The current boundary is intentional because Moonlight Web Stream is GPL-3.0-or-later and large enough that wmux should treat it as an external process unless the project deliberately accepts a license and maintenance change. macOS Sunshine has been observed wedging its GameStream HTTPS listener after a failed/stalled stream cleanup, which leaves TCP open but stalls TLS; wmux can detect this through the gateway host/app APIs, but it does not yet supervise or restart Sunshine automatically. A complete per-platform wmux agent should manage permissions, richer reconnect/status reporting, native display APIs, and first-class Sunshine/Moonlight lifecycle control.
