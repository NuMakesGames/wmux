...
- New panes receive `WMUX_COLOR_SCHEME` and `WMUX_COLOR_MODE`; browser terminals answer bounded OSC 4/10/11 queries from the live selected palette. Keep replay display-only and never let a partial replay query consume live output or send a stale response.
