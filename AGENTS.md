...
- Windows SSH PowerShell is validated on dogfood Windows hosts. The experimental Windows session agent prefers pywinpty-backed ConPTY, falls back to terminal-normalized stdio when pywinpty is unavailable, contains each pane in a kill-on-close Windows Job Object, supports staged-update draining, and records size-aware replay from agent 0.9 onward. Older agents require a best-effort 80x24 replay fallback after a wmux restart. Broad full-screen app validation and process preservation across unexpected/forced Windows-agent restarts are still pending.
